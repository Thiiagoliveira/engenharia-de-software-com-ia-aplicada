  ...
  flyTo(opts) {
    ...
    this.timeline.to(this.position, tweenSeconds, {
      ...
      onStart: () => {
        ...
        this.play();
        const requestedState = (direction || '').replace('bottom', 'top');
        const validStates = new Set(['left', 'right', 'top-left', 'top-right']);
        this.state = validStates.has(requestedState) ? requestedState : 'top-right';
        options.onStart();
      },
